...
- **Which runner owns a file is decided by its extension**: `*.test.js` is Vitest, `*.spec.js` is Playwright. Both configs pin this explicitly (Playwright's default `testMatch` would otherwise claim `*.test.js` too), so the e2e harness can keep unit tests beside its specs — `e2e/e2e-database.js` has `e2e/e2e-database.test.js`. Pick the wrong extension and the file still runs, just under the wrong runner: a `.test.js` in `e2e/` gets no browser, and a `.spec.js` under `src/` is picked up by neither.
- **Vitest** — unit/integration tests co-located as `src/**/*.test.js`, plus `e2e/**/*.test.js` for the e2e harness's own helpers. `src/app/actions.test.js` verifies every mutating Server Action is auth-gated (Prisma and the session helper are mocked, so no database is needed).
...
- **e2e never touches your development database.** Both configs run against a throwaway one, derived from `DATABASE_URL` by suffixing the name (`dinkmaster_e2e` for `pnpm test:e2e`, `dinkmaster_e2e_offline` for the offline config, so the two can run at once). Each suite has its own escape hatch — `E2E_DATABASE_URL` and `E2E_OFFLINE_DATABASE_URL` — and either is **refused if it resolves to your development database**, since setup empties whatever it is given. `e2e/global-setup.js` creates it on first run, migrates it, and **empties it before every run**, so the suite can never be slowed by its own history. A failed run leaves its rows behind for inspection — they're cleared on the way in, not on the way out.
- **Ports and build directories are deliberately separate**: `pnpm dev` owns 3020 and `.next`, `pnpm test:e2e` starts its own server on 3022 building into `.next-e2e`, and the offline config uses 3021 and `.next-e2e-offline`. Neither suite reuses a running server. The separate build directory is what lets a suite run *while* `pnpm dev` is up — Next refuses a second `next dev` for the same project directory whatever port it gets — and it keeps an offline production build from overwriting the `.next` your dev server is serving. The first run after a fresh checkout compiles every route from cold and is noticeably slower; later runs reuse the directory. A reused dev server would hold the *development* database connection and whatever modules it compiled before your last edit, so the run would either write to the wrong database or test stale code — both of which look like product bugs rather than setup mistakes.
...
DinkMaster is an installable PWA with offline support, built on [Serwist](https://serwist.pages.dev/) via `@serwist/turbopack` (keeps the build on Turbopack — no `--webpack`).

- **Service worker** — source at `src/app/sw.js`, bundled and served at `/serwist/sw.js` by the route handler in `src/app/serwist/[path]/route.js`. Registered by `SerwistProvider` in `src/app/layout.js`, and **disabled in development** so you never fight stale caches.
- **Manifest** — generated by `src/app/manifest.js` (App Router metadata API), served at `/manifest.webmanifest`. `start_url` is `/arenas`.
- **Caching** — cache-first for `_next/static` and images; stale-while-revalidate for fonts; `/login` + `/register` are network-first. Everything user-specific (`/api/*` and all authenticated navigations) is **network-only and never cached**, so one user's data can't be replayed to another on a shared device. An uncached `/arena/[id]` document request falls back to the precached `/offline-board` shell (see below); every other uncached document falls back to the precached `/offline` page.
- **Precache entries**: `additionalPrecacheEntries` (in `src/app/serwist/[path]/route.js`) lists **routes only** (`/offline`, `/offline-board`). Files under `public/` are already in the injected manifest with content-hash revisions; adding one there again with a different revision makes the Serwist constructor throw a conflicting-entries error and the worker fails to register.
- **Updates** — the worker uses `skipWaiting: false`; `SwUpdatePrompt` surfaces a banner and reloads only when the user accepts.
- **Icons** — edit `public/icons/icon-source.png`, then run `pnpm pwa:icons` to regenerate the `any` + `maskable` + Apple-touch set.

To verify locally, run a production build (`pnpm build && pnpm start`) — the SW is off under `pnpm dev` — then check DevTools → Application → Manifest / Service Workers.

### Offline session mode

A manager can keep running open play with no connection: check players in and out, add walk-ins, fill courts, record scores, skip, and shuffle. Changes apply locally and sync when the connection returns.

- **How it works**: the board mutations live as pure logic in `src/lib/board-apply.js` (server) and `src/lib/board-engine.js` (an isomorphic client reducer over the `getState` shape). Offline, the manager's actions run through the reducer and append to a per-arena **event log** in IndexedDB (`src/lib/offline-store.js`), each event recording the choices the server can't reproduce deterministically (shuffle orders, matchup tie-breaks). The board is also snapshotted to IndexedDB so `/offline-board` can render it read-only, and so the log has a base to replay over. Entry is never silent: a "connection lost" prompt offers it, and the whole flow lives in `src/app/arena-offline.js`.
- **Shared-device safety**: IndexedDB holds only board data that is already public via the arena page and its SSE stream, plus the viewer's own role flags. No authenticated HTML is ever cached: the `/offline-board` shell is neutral static markup that renders the board client-side from IndexedDB.
- **Sync**: on reconnect the log replays through `syncOfflineEvents` (in `src/app/actions.js`), one transaction under the arena's queue lock. It is idempotent (the client batch id is the primary key of `OfflineSyncBatch`), and a board **fingerprint** (`src/lib/board-fingerprint.js`) captured at entry is recompared server-side to detect divergence. Strict mode rolls the whole batch back on any mismatch and prompts the manager; best-effort applies what still fits and reports the rest. Recorded match times carry through to `Match.createdAt`, so the weekly leaderboard keys off court time, not sync time.
- **Advisory hold**: when a manager enters offline mode while still reachable, other viewers get a soft "X is running the board offline" notice (`Arena.offlineHolderLabel` / `offlineHeldAt`, broadcast via the SSE stream). It's never enforced (a truly offline device can't declare it), is cleared when the session syncs, and expires client-side after 8h so a lost device's notice doesn't linger.

Read-only offline scope: only `/arena/[id]` has an offline shell. Visiting `/arenas` (the directory) offline still shows the generic offline page; the `/offline-board` route itself lists every arena saved for offline use.

## Data model

Defined in [`prisma/schema.prisma`](prisma/schema.prisma):

- **Arena** — an isolated session owned by a `User`. Players, courts, matches, and partnerships are all scoped by `arenaId`. Carries an optional `description` blurb, a recurring **schedule** (`scheduleDays` 0–6, `scheduleStart`/`scheduleEnd` `"HH:MM"`, `timezone`) used for the Mon–Sun **Player of the Week** window, **matchmaking thresholds** (`starveThreshold` / `emergencyWait`) controlling the auto-mix bands and the ⏳ badge, and **match + leaderboard defaults** (`targetScore`, `autoMixDefault`, `leaderboardSize`, `countOffScheduleGames`) seeding the score modal, the auto-mix toggle, and the **This Week** board. `offlineHolderLabel` / `offlineHeldAt` back the advisory offline-hold notice (see [Offline session mode](#offline-session-mode)).
...
- **OfflineSyncBatch**: idempotency ledger for offline session sync. Its id is the client-generated batch id, so a retried sync that already committed returns current state instead of double-applying. No FK to `Arena`, so a late retry for a deleted arena still dedupes cleanly.
