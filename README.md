...
| **9b — Match & leaderboard defaults** | Per-arena overrides for **target score** (score-modal seed), **auto-mix default** (initial toggle state), **leaderboard top-N** (size of the **This Week** board), and whether **off-schedule games count** (restricts the weekly tally to scheduled days when off). Threaded into the score modal, `endMatch`, and `computeWeeklyLeaderboard`; settable from **Arena Settings → Match Defaults**. Defaults preserve prior behaviour. Team size stays fixed at doubles (2v2). | 🚧 In progress |
