...
describe('computeWeeklyLeaderboard', () => {
  ...
  it('counts only matches on scheduled weekdays', () => {
    const matches = [
      match(['a', 'x'], ['b', 'y'], 1, 18), // Monday — counts
      match(['a', 'x'], ['b', 'y'], 1, 20), // Wednesday — excluded
    ];
    // schedule.days uses JS getDay(): Monday = 1.
